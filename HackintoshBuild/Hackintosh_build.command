...
liluPlugins='AirportBrcmFixup AppleALC ATH9KFixup BT4LEContinuityFixup CPUFriend HibernationFixup NoTouchID RTCMemoryFixup SystemProfilerMemoryFixup VirtualSMC acidanthera_WhateverGreen bugprogrammer_WhateverGreen NVMeFix'
...
for i in ${selectedArray[*]}; do
    ...
    if [ ${buildArray[$i]%,*} == 'Clover' ]; then
        ...
        echo 6 | ./buildme > /dev/null || exit 1
        cp -Rf CloverPackage/sym/* ../../Release/${buildArray[$i]%,*}/Release > /dev/null || exit 1
        echo ${buildArray[$i]%,*}"编译成功"
        rm -rf ~/Desktop/$dir/Release/${buildArray[$i]%,*}/Debug > /dev/null || exit 1
    elif [[ $bootLoader =~ ${buildArray[$i]%,*} ]]; then
        ./*.tool > /dev/null || exit 1
        cp Binaries/RELEASE/*.zip ../../Release/${buildArray[$i]%,*}/Release > /dev/null || exit 1

        cp Binaries/DEBUG/*.zip ../../Release/${buildArray[$i]%,*}/Debug > /dev/null || exit 1
        ...
        if [[ $liluPlugins =~ ${buildArray[$i]%,*} ]]; then
            if [ ! -e *.kext ]; then
                ...
                if [ ! -e $url/$dir/Sources/Lilu/build/Debug/Lilu.kext ]; then
                    ...
                    popd
                    cp -Rf $url/$dir/Sources/Lilu/build/Debug/Lilu.kext . > /dev/null || exit 1
                else
                    cp -Rf $url/$dir/Sources/Lilu/build/Debug/Lilu.kext . > /dev/null || exit 1
                fi
            fi
        ...
        if [ -e build/Release/*.zip ]; then
            cp -Rf build/Release/*.zip ../../Release/${buildArray[$i]%,*}/Release > /dev/null || exit 1
            cp -Rf build/Debug/*.zip ../../Release/${buildArray[$i]%,*}/Debug > /dev/null || exit 1
            echo ${buildArray[$i]%,*}"编译成功"
        else
            cp -Rf build/Release/*.kext ../../Release/${buildArray[$i]%,*}/Release/${buildArray[$i]%,*}-Release.kext > /dev/null || exit 1
            cp -Rf build/Debug/*.kext ../../Release/${buildArray[$i]%,*}/Debug/${buildArray[$i]%,*}-Debug.kext > /dev/null || exit 1
            echo ${buildArray[$i]%,*}"编译成功"
        fi
    fi
    popd
