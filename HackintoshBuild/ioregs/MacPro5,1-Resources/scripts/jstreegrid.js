...
(function ($) {
	...
	$.jstree.plugin("grid", {
		__init : function () { 
			...
			// Blackosx - commented the following out for now because $.browser was dropped in jQuery 1.9
			// I need to find an alternative to use.
			...
			_prepare_headers : function() {
				...
				for (i=0;i<cols.length;i++) {
					...
					last = $("<div></div>").css(conf).css({"margin-left": margin,"width":width, "padding": "1 3 2 5"}).addClass((tr?"ui-widget-header ":"")+"jstree-grid-header jstree-grid-header-cell jstree-grid-header-"+classAdd+" "+cl).text(val).appendTo(header)
						...
						//.before("<div class='jstree-grid-separator jstree-grid-separator-"+classAdd+(tr ? " ui-widget-header" : "")+(resizable? " jstree-grid-resizable-separator":"")+"'>&nbsp;</div>");
				}
					...
					if (a.length === 1) {
					  a.prev().css("float","left");
						...
						for (i=1;i<cols.length;i++) {
							...
							//span.addClass(cl+" "+valClass).css({"margin-right":"0px","display":"inline-block","*display":"inline","*+display":"inline","cursor":"pointer"}}).html(content).mouseover((function (val,col,s) {
